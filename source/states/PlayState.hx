...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['UR IQ is Erpin?', 0.2], //From 0% to 19%
		['Ughhh...', 0.4], //From 20% to 39%
		['Bed', 0.5], //From 40% to 49%
		['Brah', 0.6], //From 50% to 59%
		['Mah', 0.69], //From 60% to 68%
		['Not so tasty', 0.7], //69%
		['Yum...?', 0.8], //From 70% to 79%
		['Yum Yum!', 0.9], //From 80% to 89%
		['Dellciuus!!', 1], //From 90% to 99%
		['BBAAAAANG!!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public var canDeath:Bool = false; // bf can died?

	public var shakingWindows:ShakingWindows = new ShakingWindows(); // shakings

	// dialogue (Lua (i love you NOOB1, very thanks))
	public var textSize:String = 'Middle';
	public var textSpeed:String = 'Fast';
	public var interaction:Bool = true;
	...
	public var detailsResultText:String = "";
	#end
	...
	override public function create()
	{
		...
		playbackRate = ClientPrefs.getGameplaySetting('songspeed', ClientPrefs.defaultData.gameplaySettings.get('songspeed'));
		...
		// Gameplay settings
		healthGain = ClientPrefs.getGameplaySetting('healthgain', ClientPrefs.defaultData.gameplaySettings.get('healthgain'));
		healthLoss = ClientPrefs.getGameplaySetting('healthloss', ClientPrefs.defaultData.gameplaySettings.get('healthloss'));
		instakillOnMiss = ClientPrefs.getGameplaySetting('instakill', ClientPrefs.defaultData.gameplaySettings.get('instakill'));
		practiceMode = ClientPrefs.getGameplaySetting('practice', ClientPrefs.defaultData.gameplaySettings.get('practice'));
		cpuControlled = ClientPrefs.getGameplaySetting('botplay', ClientPrefs.defaultData.gameplaySettings.get('botplay'));
		guitarHeroSustains = ClientPrefs.data.guitarHeroSustains;

		// dialogue (Lua (i love you NOOB1, very thanks))
		textSize = ClientPrefs.data.textSize;
		textSpeed = ClientPrefs.data.textSpeed;
		interaction = ClientPrefs.data.interaction;
		...
		// String for when the game is show results
		detailsResultText = "Result - " + detailsText;
		#end
		...
		timeTxt.setFormat(Paths.font("mobileone.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("mobileone.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("mobileone.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		blackThing = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, 0xFF000000);
		blackThing.alpha = 0;
		blackThing.scrollFactor.set();
		blackThing.cameras = [FlxG.cameras.list[FlxG.cameras.list.length - 1]];
		add(blackThing);
	}
	...
	override public function update(elapsed:Float)
	{
		...
		shakingWindows.updateShake(elapsed);
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && gameOverTimer == null && canDeath)
		{
		...
		else if ((skipHealthCheck && instakillOnMiss) && !practiceMode && !isDead && gameOverTimer == null && !canDeath)
		{
			var ret:Dynamic = callOnScripts('onGameOver', null, true);
			if(ret != LuaUtils.Function_Stop)
			{
				...
				#if DISCORD_ALLOWED
				// Game Over doesn't get his its variable because it's only used here
				if(autoUpdateRPC) DiscordClient.changePresence("Resetting - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
	...
	function openResultScreen(callback:Void->Void) {
		persistentUpdate = false;
		...
		openSubState(new ResultSubState(boyfriend, callback));
	}
	...
	function noteMissCommon(direction:Int, note:Note = null)
	{
		...
		if (!canDeath && health <= 0 && !shakingWindows.shaking)
			shakingWindows.startWindowShake();
	}
