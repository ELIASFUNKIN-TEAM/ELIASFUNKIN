...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	override function create()
	{
		...
		var screenX = FlxG.camera.scroll.x;
		var screenY = FlxG.camera.scroll.y;

		var butterMincha:FlxSprite = new FlxSprite().loadGraphic(Paths.image('easteregg/buttermincha'));
		butterMincha.alpha = 0;
		butterMincha.setGraphicSize(FlxG.width, FlxG.height);
		butterMincha.updateHitbox();
		butterMincha.setPosition(0, 0);
		butterMincha.scrollFactor.set();
		butterMincha.cameras = [camUI];

		var minchaButton:ButtBaseThing = new ButtBaseThing(
			0,
			0,
			'',
			'null'
		);
		minchaButton.ckedGraphic = Paths.image('easteregg/checked-mincha');
		minchaButton.unckedGraphic = Paths.image('easteregg/mincha');
		minchaButton.onClickingCallback = function() {
			if (!minchaButton.isChecked)
				minchaButton.scaleTween = FlxTween.tween(minchaButton, {'scale.x': 1.15, 'scale.y': 0.75}, 0.015);
		}
		minchaButton.onClickedUpCallback = function() {
			if (!minchaButton.isChecked) {
				minchaButton.scaleTween = FlxTween.tween(this, {'scale.x': 1, 'scale.y': 1}, 0.015);
				FlxTween.tween(butterMincha, {alpha: 1}, 0.015, {
					ease: FlxEase.linear,
					onStart: function(_) {
						var sound = FlxG.sound.play(Paths.sound('scrollMenu'));
						sound.pitch = 0.35;
						sound.endTime = sound.length + 11000;
					},
					onComplete: function(_) {
							new FlxTimer().start(1,
							function(_) {
								FlxTween.tween(butterMincha, {alpha: 0}, 1);
							}
						);
					}
				});
		...
		minchaButton.onOverlapCallback = function() minchaButton.alpha = 1;
		minchaButton.onOutCallback = function() {
			minchaButton.scaleTween = FlxTween.tween(this, {'scale.x': 1, 'scale.y': 1}, 0.015);
			minchaButton.alpha = 0.3;
			minchaButton.isChecked = false;
		}
		minchaButton.alpha = 0.3;
		minchaButton.setPosition(FlxG.width - minchaButton.width - 30, FlxG.height - minchaButton.height - 30);
		minchaButton.isChecked = false;
