...
class Paths
{
	...
	static public function image(key:String, ?parentFolder:String = null, ?allowGPU:Bool = true, ?modsAllowed:Bool = true):FlxGraphic
	{
		...
		return cacheBitmap(key, parentFolder, bitmap, allowGPU, modsAllowed);
	}

	public static function cacheBitmap(key:String, ?parentFolder:String = null, ?bitmap:BitmapData, ?allowGPU:Bool = true, ?modsAllowed:Bool = true):FlxGraphic
	{
		if (bitmap == null)
		{
			var file:String = getPath(key, IMAGE, parentFolder, modsAllowed);
			#if MODS_ALLOWED
